...
switch (cmd)
{
    case "bench":
        Run(
            "dotnet",
            "run --project src/Maple.Models.Benchmarks/Maple.Models.Benchmarks.csproj"
    ...
    case "comparison-bench":
        Run(
            "dotnet",
            "run --project src/Maple.Models.ComparisonBenchmarks/Maple.Models.ComparisonBenchmarks.csproj"
                + " -c Release -- run",
            repoRoot
        );
        break;
    ...
    case "disasm":
        Run(
            "dotnet",
            "run --project src/Maple.Models.Benchmarks/Maple.Models.Benchmarks.csproj"
                + " -c Release -- --filter \"*MapleModels*\" --disasm",
            repoRoot
        );
        break;

    case "test":
        // dotnet test + TUnit on .NET 10 SDK requires running the test exe directly (MTP 2.1.0 VSTest check)
        var cfg = args.Length > 1 ? args[1] : "Release";
        Run("dotnet", $"run --project src/Maple.Models.Test/Maple.Models.Test.csproj -c {cfg}", repoRoot);
        Run("dotnet", $"run --project src/Maple.Models.XyzTest/Maple.Models.XyzTest.csproj -c {cfg}", repoRoot);
        break;
    ...
    case "publish-tester":
        ...
        Run(
            "dotnet",
            $"publish src/Maple.Models.Tester/Maple.Models.Tester.csproj -c Release -r {rid} --self-contained",
            repoRoot
        );
        break;
    ...
    default:
        Console.WriteLine(
            ...
            Commands:
              test [Debug|Release]       Run all tests (default: Release)
